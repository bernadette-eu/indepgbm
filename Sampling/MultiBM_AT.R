...
cov_data <- list(A           = age_bands,
                 ...
                 # Prior distributions:
                 eta0_sd   = rep(1, age_bands),
                 eta1_sd   = rep(1, age_bands),
                 p_sigmaCM = 0.05,
...
#---- Set initial values:
fit_optim <- rstan::optimizing(Bernadette:::stanmodels$seeiir_mbm_ncp_gamma_volatilities, 
                               data    = cov_data, 
                               seed    = 1, 
                               hessian = TRUE)

sampler_init <- function(){
  list(eta0_u    = fit_optim$par[grepl("eta0_u",     names(fit_optim$par), fixed = TRUE)],
       eta1_u    = fit_optim$par[grepl("eta1_u",     names(fit_optim$par), fixed = TRUE)],
       ...
       disp      = fit_optim$par[grepl("disp",       names(fit_optim$par), fixed = TRUE)]
  )
}# End function
...
nuts_fit_1 <- rstan::sampling(Bernadette:::stanmodels$seeiir_mbm_ncp_gamma_volatilities,
                              data     = cov_data,
                              init     = sampler_init,
                              pars     = parameters,
                              chains   = nChains,
