functions {
	...
	real[] ODE_states(real time,     // Time
      ...
      real rho            = theta[A * A + 3];                 // Number of cases at t0
	  real beta[A]        = theta[(A * A + 4):(A*A + A + 3)]; // Effective contact rate

      // Compartments:
      for (i in 1:A){
        
		init[i]         = age_dist[i]  * (1-rho); // Initial states - Susceptibles
        init[A + i]     = age_dist[i]  *  rho;    // Initial states - Exposed 1
									  ...
									  )
    {
      ...
      theta_ODE[A * A + 3] = theta[A*n_obs + A*A + 3]; // rho
...
parameters {
	...
	real x_init[A];                         // Initial transmission rate
	real x_noise[(n_changes - 1)*A];
	real<lower = 0, upper = 1> rho;            // Number of cases at t0
...
transformed parameters{
  ...
  if(doprint != 0) {
	  ...
	  print("x_init: ", x_init);
	  print("eta_noise: ", x_noise);
	  print("sigmaBM: ", sigmaBM);
	  print("beta0: ", beta0);
	  print("eta_daily: ", x_trajectory);
	  print("beta_N: ", beta_N);
...
model {
  ...
  x_init    ~ normal(0, p_sigmaBM[1]);
  x_noise   ~ std_normal();

  rho       ~ beta(p_pi[1,1], p_pi[1,2]);
...
generated quantities {
  // NOTES:
  // Quantities, useful for model comparison:
  // 1. https://cran.r-project.org/web/packages/loo/vignettes/loo2-with-rstan.html
  // 2. Connection between eigen and cholesky decomposition (requires a symmetric matrix in Stan):
  //    https://math.stackexchange.com/questions/2903455/a-simple-proof-for-the-relationship-between-the-eigenvalues-of-a-positive-defini
  ...
  vector[n_obs] log_lik;             // Log-likelihood vector for loo package. Need the name "log_lik", for use by the function loo::extract_log_lik().
                                     // Guidelines: https://mc-stan.org/loo/articles/loo2-with-rstan.html
  ...
  for (i in 1:n_obs) {
	...
	for (j in 1:A) {
	 	Susceptibles[i,j] = ( state_solutions[i,j] + (age_dist[j]  * (1-rho)) )* n_pop ;
		log_like_age[i,j] = neg_binomial_2_lpmf(y_deaths[i,j] | E_deathsByAge[i,j], E_deathsByAge[i,j]/phiD);
    }// End for
	 
    // Log-likelihood:
    // https://github.com/stan-dev/stan/issues/2734
